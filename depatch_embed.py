...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, patch_count=14, in_chans=3, embed_dim=768, with_norm=False):
        super().__init__()  
        patch_stride = img_size // patch_count # 16
        patch_pad = (patch_stride * (patch_count - 1) + patch_size - img_size) // 2 # 0
    ...
    def forward(self, x, **kwargs):
        ...
        x = self.proj(x).flatten(2).transpose(1, 2) # Return (B, numpatches, embed_dim)
...
class Simple_Patch(nn.Module):
    ...
    def forward(self, x, model_offset=None):
        ...
        return self.get_output(img, pred_offset, model_offset), (self.patch_count, self.patch_count) # Embedding rồi mới predict
                
                
class Simple_DePatch(Simple_Patch):
    def __init__(self, box_coder, show_dim=4, **kwargs):
        super().__init__(show_dim, **kwargs)
        self.box_coder = box_coder # The predicted boxes
        ...
        self.num_sample_points = self.patch_pixel * self.patch_pixel * self.patch_count * self.patch_count # Số điểm được sample với patch_pixel là số pixels được samples ở mỗi chiều 
        if kwargs["with_norm"]:
            self.with_norm=True
    ...
    def get_output(self, img, pred_offset, model_offset=None):
        ...
        # output_proj
        output = output.view(B, self.num_patches, self.in_chans*self.patch_pixel*self.patch_pixel) # Finally: (B, num_patches, in_chans * patch_pixel ^ 2)
