...
class pointCoder(nn.Module):
    ...
    # Generate x,y centor for the number of anchor boxes(patch_count ^ 2 centers)
    def _generate_anchor(self):
        ...
        for i in range(self.patch_count):
            for j in range(self.patch_count):
                ...
                anchors.append([x, y])
        anchors = torch.as_tensor(anchors) # (patch_count^2, 2)
        self.register_buffer("anchor", anchors) # Lưu thành một biến hyperparameter trong class
    ...
    def decode(self, rel_codes):
        # Tính toán bounding cho từng patch với giá trị (x,y) trong đoạn [0,1]
        # print ('xyxy decoding')
        boxes = self.anchor
        pixel = 1./self.patch_count
        wx, wy = self.weights

        dx = F.tanh(rel_codes[:, :, 0]/wx) * pixel if self.tanh else rel_codes[:, :, 0]*pixel / wx # rel_codes.shape = patch_count * patch_count * 2
        dy = F.tanh(rel_codes[:, :, 1]/wy) * pixel if self.tanh else rel_codes[:, :, 1]*pixel / wy

        pred_boxes = torch.zeros_like(rel_codes)

        ref_x = boxes[:,0].unsqueeze(0) # Add an dimension of 1 at position 0
        ref_y = boxes[:,1].unsqueeze(0)

        pred_boxes[:, :, 0] = dx + ref_x # New center
        pred_boxes[:, :, 1] = dy + ref_y # New center
...
class pointwhCoder(pointCoder):
    ...
    def forward(self, pts, model_offset=None):
        ...
        return points # Final Return is the address of the sampled points of each batch
    ...
    def decode(self, rel_codes):
        ...
        return pred_boxes # All predicted boxes
    ...
    def meshgrid(self, boxes): # Sample uniformly patch_pixel ^ 2 points, return the locations
        B = boxes.shape[0]
        xs, ys = boxes[:, :, 0::2], boxes[: , :, 1::2] # xs là đoạn thẳng chứa 2 đầu của bounding box chiều x, tương tự với ys
        xs = torch.nn.functional.interpolate(xs, size=self.patch_pixel, mode='linear', align_corners=True) # Sample xs
        ys = torch.nn.functional.interpolate(ys, size=self.patch_pixel, mode='linear', align_corners=True) # Sample ys
